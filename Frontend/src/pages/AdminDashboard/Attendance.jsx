...
export default function Attendance() {
  ...
  const fetchRecords = async () => {
    try {
      const res = await fetch(`${API}/Admin/attendance/entry/details`);
      const data = await res.json();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const url = editId ? `${API}/Admin/attendance/entry/update` : `${API}/Admin/attendance/entry/add`;
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        setForm({ Name: "", EntryType: "MANUAL", Status: "Present" });
        setEditId(null);
        fetchRecords();
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API}/Admin/attendance/entry/delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
      ...
      <form onSubmit={handleSubmit} className="dashboard-form">
        ...
        <div className="form-group" style={{ flexDirection: "row", alignItems: "flex-end" }}>
          <button type="submit" className="dash-btn" disabled={loading}>
            {loading ? "Saving..." : editId ? "Update Log" : "Record Entry"}
          </button>
          {editId && (
          ...
          <div className="filter-hub" style={{ marginTop: '40px' }}>
              ...
              <div className="filter-row">
                  ...
                  <div className="filter-group">
                      <label className="filter-label">Log Status</label>
                  ...
                  {(searchTerm || yearFilter !== "All" || deptFilter !== "All" || genderFilter !== "All" || statusFilter !== "All" || roomFilter !== "All") && (
